...
"""
# sutranets/ml/add_args.py

from sutranets.file_utils import MyArgumentTypes
from sutranets.ml.constants import IntraBinDecoder
from sutranets.utils import add_map_args, add_arg_triple, add_window_args
...
def add_option_args(parser, *, add_train_args=True, add_test_args=True):
    ...
    if add_train_args:
        ...
        parser.add_argument('--bin_dropout', type=float, required=False,
                            help="How much dropout to apply to input bins.")
    if add_test_args:
...
def add_set_args_by_label(parser, label, require_loss_start=True,
    ...
    parser.add_argument(f'--{label}_truncate_pt_s', type=int, required=False,
                        help=f"If given, exclude all points from here on in {label}.")
...
def add_standard_args(parser, *, add_train_args=True, add_test_args=True):
    ...
    parser.add_argument('--seed', type=int, required=False,
                        help="If given, use this random seed for this run, otherwise, no seed.")
...
def add_standard_train_args(parser):
    ...
    parser.add_argument('--batch_subset_frac', type=float, required=False, default=1.0,
                        help="Only use this fractions of batches during each iteration.")
